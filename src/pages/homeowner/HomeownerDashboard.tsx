  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const activeJobs = jobsData?.filter(job => job.status === 'open' || job.status === 'in_progress').length || 0;
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        <div>
          <h1 className="text-2xl font-bold">Welcome back, {profile?.first_name || 'Homeowner'}</h1>
                ...
                recentJobs.map((job) => (
                  <div key={job.id} className="p-6">
                    <div className="flex justify-between items-start mb-2">
                      <Link to={`/homeowner/job/${job.id}`} className="text-lg font-medium hover:text-blue-500">
                        {job.title}
                      </Link>
                    ...
                    <div className="flex items-start">
                      <div className="h-10 w-10 rounded-full bg-gray-300 flex items-center justify-center text-white mr-3">
                        {message.sender.avatar_url ? (
                          <img src={message.sender.avatar_url} alt="Sender" className="h-10 w-10 rounded-full" />
                        ) : (
